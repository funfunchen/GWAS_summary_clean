...
z_trans <- function(x, y){
  ...
  #
  #Args:
  #  x: vectors of effect sizes on Traint X (Exposure)
  #  y: vectors of effect sizes on Traint Y (Outcome)
  #
  #Result:
  #  z: z hat
...
lh_M <- function(x_hat, x, n_x, y_hat, y, n_y){
  ...
  #
  #Args:
  #  x_hat: z hat of trait X data set (Exposure)
  #  x:  trait X data set
  #  n_x: trait X data set (to get the length)
  #  y_hat: z hat of trait Y data set (Outcome)
  #  y: trait X data set
...
for (yu in yu.files){
  ...
  write.table(format(my_res, digits = 3), paste0(trait.name,".2way.full"), quote=F, row.names=F, sep="\t")
  write.table(format(my_res_sig, digits = 3), paste0(trait.name,".2way.sig"), quote=F, row.names=F, sep="\t")
}
